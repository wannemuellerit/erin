...
class ScanCompanyMedia implements ShouldQueue
{
    use Queueable;
    ...
    {
        $media = CompanyMedia::query()->findOrFail($this->mediaId);
        $stream = Storage::disk($media->disk)->readStream($media->path);

        if (! is_resource($stream)) {
            throw new RuntimeException('Firmenmedium konnte nicht gelesen werden.');
        }
    ...
    {
        CompanyMedia::query()->whereKey($this->mediaId)->update([
